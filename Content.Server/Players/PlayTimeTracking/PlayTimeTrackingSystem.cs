...
public sealed class PlayTimeTrackingSystem : EntitySystem
{
    ...
    public bool IsAllowed(ICommonSession player, string role)
    {
        ...
        return JobRequirements.TryRequirementsMet(job, playTimes, out _, EntityManager, _prototypes, (HumanoidCharacterProfile?) _preferencesManager.GetPreferences(player.UserId).SelectedCharacter, _cfg.GetCVar(CCVars.GameRoleTimers) ? _cfg.GetCVar(CCVars.GameRoleTimersMultiplier) : 0f, isWhitelisted);
    }
    ...
    public HashSet<ProtoId<JobPrototype>> GetDisallowedJobs(ICommonSession player)
    {
        ...
        {
            if (JobRequirements.TryRequirementsMet(job, playTimes, out _, EntityManager, _prototypes, (HumanoidCharacterProfile?) _preferencesManager.GetPreferences(player.UserId).SelectedCharacter, roleTimersMultiplier, isWhitelisted))
                roles.Add(job.ID);
        }
    ...
    public void RemoveDisallowedJobs(NetUserId userId, List<ProtoId<JobPrototype>> jobs)
    {
        ...
        for (var i = 0; i < jobs.Count; i++)
        {
            if (_prototypes.TryIndex(jobs[i], out var job)
                && JobRequirements.TryRequirementsMet(job, playTimes, out _, EntityManager, _prototypes, (HumanoidCharacterProfile?) _preferencesManager.GetPreferences(userId).SelectedCharacter, roleTimersMultiplier, isWhitelisted))
            {
                continue;
            }
